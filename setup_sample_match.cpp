...
int main() {
  ...
  string sample_media = app_dir + (string)"\\Sample\\media",
    sample_saved = app_dir + (string)"\\Sample\\saved_data\\SampleMatch";
  ...
  if (!dir_exists(sample_media) || !dir_exists(sample_saved)) {
    ...
    cout << "Setting up server data" << endl << endl;
    ...
    string dest_media = app_dir + (string)"\\user_data\\media",
      dest_saved = app_dir + (string)"\\user_data\\saved_data\\SampleMatch";
  ...
  if (dir_exists(app_dir + (string)"\\Sample\\SampleMatch")) {
    cout << endl << "Sample cache directories:" << endl;
    cout << " > " << sample_cache_player << endl;
    cout << " > " << sample_cache_other << endl << endl;

    cout << "Setting up client data" << endl;

    string dest_parent = app_dir + (string)"\\user_data\\match_data",
      dest_match = dest_parent + "\\SampleMatch",
      dest_other = dest_match + "\\other",
      dest_player = dest_match + "\\player";

    cout << " > Path: " << dest_match << endl << endl;

    if (!dir_exists(dest_parent)) {
      _mkdir(dest_parent.c_str());
    }
    if (!dir_exists(dest_match)) {
      _mkdir(dest_match.c_str());
    }
    ...
    ofstream cache_file(app_dir + (string)"\\user_data\\match_data\\cache.txt");
